  ...
  <div class="page-content row">
    ...
    <div class="page-content-wrapper">
	  ...
	  <div class="toolbar-line">
			...
			@if($access['is_add'] ==1)
	   		<a href="{{ URL::to('blogadmin/add') }}" class="tips btn btn-xs btn-primary"  title="{{ Lang::get('core.btn_create') }}">
			...
			<a href="{{ URL::to('blogadmin/download') }}" class="tips btn btn-xs btn-default" title="{{ Lang::get('core.btn_download') }}">
			<i class="icon-folder-download2"></i>&nbsp;{{ Lang::get('core.btn_download') }} </a>
			@endif		
		 	@if(Session::get('gid') ==1)
			<a href="{{ URL::to('module/config/blogadmin') }}" class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_config') }}">
            ...
            @foreach ($rowData as $row)
                <tr>
				 ...
				 <td>
					...
					<a href="{{ URL::to('blogadmin/show/'.$id)}}"  class="tips btn btn-xs btn-default"  title="{{ Lang::get('core.btn_view') }}"><i class="fa fa-search"></i> </a>
					@endif
					@if($access['is_edit'] ==1)
					<a  href="{{ URL::to('blogadmin/add/'.$id)}}"  class="tips btn btn-xs btn-success"  title="{{ Lang::get('core.btn_edit') }}"> <i class="fa fa-edit"></i></a>
					@endif
